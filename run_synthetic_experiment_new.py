...
if __name__ == "__main__":
    ...
    f_max_dict = torch.load(os.path.join(cfg_data["out_dir"], "f_max.pt")).copy()
    train_x_dict = torch.load(os.path.join(cfg_data["out_dir"], "train_x.pt")).copy()
    train_y_dict = torch.load(os.path.join(cfg_data["out_dir"], "train_y.pt")).copy()
    lengthscales_dict = torch.load(os.path.join(cfg_data["out_dir"], "lengthscales.pt")).copy()
    ...
    for dim in cfg_data["dimensions"]:
        ...
        for index_objective in range(cfg_data["num_objectives"]):
            ...
            true_max = f_max_dict[dim][index_objective]
            print(f"Optimizer's max reward: {max(rewards)}")
            print(f"Optimizer's normalized reward: {(true_max - max(rewards)) / true_max}\n")
